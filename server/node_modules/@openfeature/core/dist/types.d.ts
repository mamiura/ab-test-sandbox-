...
type ProviderEntryInput<TProvider> = {
    provider: TProvider;
    name?: string;
};
type RegisteredProvider<TProvider> = Required<ProviderEntryInput<TProvider>>;
...
declare const throwAggregateErrorFromPromiseResults: <TProvider>(result: PromiseSettledResult<unknown>[], providerEntries: Required<ProviderEntryInput<TProvider>>[]) => void;
 ...
 */
declare class StatusTracker<TProviderEvents extends AnyProviderEvent, TProviderStatus, TProvider extends {
    events?: ProviderEventEmitter<TProviderEvents>;
}> {
    private events;
    private statusEnum;
    private eventEnum;
    private readonly providerStatuses;
    constructor(events: ProviderEventEmitter<TProviderEvents>, statusEnum: ProviderStatusEnum<TProviderStatus>, eventEnum: ProviderEventsEnum<TProviderEvents>);
    wrapEventHandler(providerEntry: RegisteredProvider<TProvider>): void;
    providerStatus(name: string): TProviderStatus;
    private getStatusFromProviderStatuses;
    private changeProviderStatus;
}
...
type BaseStrategyProviderContext<TProviderStatus, TProvider> = {
    provider: TProvider;
    providerName: string;
    providerStatus: TProviderStatus;
};
type BaseStrategyPerProviderContext<TProviderStatus, TProvider> = StrategyEvaluationContext & BaseStrategyProviderContext<TProviderStatus, TProvider>;
type BaseProviderResolutionResultBase<TProvider> = {
    provider: TProvider;
    providerName: string;
};
type BaseProviderResolutionSuccessResult<T extends FlagValue, _TProviderStatus, TProvider> = BaseProviderResolutionResultBase<TProvider> & {
    details: ResolutionDetails<T>;
};
type BaseProviderResolutionErrorResult<_TProviderStatus, TProvider> = BaseProviderResolutionResultBase<TProvider> & {
    thrownError: unknown;
};
type BaseProviderResolutionResult<T extends FlagValue, TProviderStatus, TProvider> = BaseProviderResolutionSuccessResult<T, TProviderStatus, TProvider> | BaseProviderResolutionErrorResult<TProviderStatus, TProvider>;
type BaseFinalResult<T extends FlagValue, _TProviderStatus, TProvider> = {
    details?: ResolutionDetails<T>;
    provider?: TProvider;
    providerName?: string;
    errors?: {
 ...
 */
declare abstract class BaseEvaluationStrategy<TProviderStatus, TProvider> {
    protected statusEnum: Record<string, TProviderStatus>;
    runMode: 'parallel' | 'sequential';
    constructor(statusEnum: Record<string, TProviderStatus>);
    shouldEvaluateThisProvider(strategyContext: BaseStrategyPerProviderContext<TProviderStatus, TProvider>, _evalContext?: EvaluationContext): boolean;
    shouldEvaluateNextProvider<T extends FlagValue>(_strategyContext?: BaseStrategyPerProviderContext<TProviderStatus, TProvider>, _context?: EvaluationContext, _result?: BaseProviderResolutionResult<T, TProviderStatus, TProvider>): boolean;
    shouldTrackWithThisProvider(strategyContext: BaseStrategyProviderContext<TProviderStatus, TProvider>, _context?: EvaluationContext, _trackingEventName?: string, _trackingEventDetails?: TrackingEventDetails): boolean;
    abstract determineFinalResult<T extends FlagValue>(strategyContext: StrategyEvaluationContext, context: EvaluationContext, resolutions: BaseProviderResolutionResult<T, TProviderStatus, TProvider>[]): BaseFinalResult<T, TProviderStatus, TProvider>;
    protected hasError<T extends FlagValue>(resolution: BaseProviderResolutionResult<T, TProviderStatus, TProvider>): resolution is BaseProviderResolutionErrorResult<TProviderStatus, TProvider> | (BaseProviderResolutionSuccessResult<T, TProviderStatus, TProvider> & {
        details: ResolutionDetails<T> & {
            errorCode: ErrorCode;
        };
    });
    protected hasErrorWithCode<T extends FlagValue>(resolution: BaseProviderResolutionResult<T, TProviderStatus, TProvider>, code: ErrorCode): boolean;
    protected collectProviderErrors<T extends FlagValue>(resolutions: BaseProviderResolutionResult<T, TProviderStatus, TProvider>[]): BaseFinalResult<T, TProviderStatus, TProvider>;
    protected resolutionToFinalResult<T extends FlagValue>(resolution: BaseProviderResolutionSuccessResult<T, TProviderStatus, TProvider>): {
        details: ResolutionDetails<T>;
        provider: TProvider;
        providerName: string;
    };
...
declare class FirstMatchStrategy<TProviderStatus, TProvider> extends BaseEvaluationStrategy<TProviderStatus, TProvider> {
    shouldEvaluateNextProvider<T extends FlagValue>(strategyContext: BaseStrategyPerProviderContext<TProviderStatus, TProvider>, context: EvaluationContext, result: BaseProviderResolutionResult<T, TProviderStatus, TProvider>): boolean;
    determineFinalResult<T extends FlagValue>(strategyContext: BaseStrategyPerProviderContext<TProviderStatus, TProvider>, context: EvaluationContext, resolutions: BaseProviderResolutionResult<T, TProviderStatus, TProvider>[]): BaseFinalResult<T, TProviderStatus, TProvider>;
}

declare class FirstSuccessfulStrategy<TProviderStatus, TProvider> extends BaseEvaluationStrategy<TProviderStatus, TProvider> {
    shouldEvaluateNextProvider<T extends FlagValue>(strategyContext: BaseStrategyPerProviderContext<TProviderStatus, TProvider>, context: EvaluationContext, result: BaseProviderResolutionResult<T, TProviderStatus, TProvider>): boolean;
    determineFinalResult<T extends FlagValue>(strategyContext: BaseStrategyPerProviderContext<TProviderStatus, TProvider>, context: EvaluationContext, resolutions: BaseProviderResolutionResult<T, TProviderStatus, TProvider>[]): BaseFinalResult<T, TProviderStatus, TProvider>;
}

declare class ComparisonStrategy<TProviderStatus, TProvider> extends BaseEvaluationStrategy<TProviderStatus, TProvider> {
    private fallbackProvider;
    private onMismatch?;
    runMode: "parallel";
    constructor(statusEnum: Record<string, TProviderStatus>, fallbackProvider: TProvider, onMismatch?: ((resolutions: BaseProviderResolutionResult<FlagValue, TProviderStatus, TProvider>[]) => void) | undefined);
    determineFinalResult<T extends FlagValue>(strategyContext: BaseStrategyPerProviderContext<TProviderStatus, TProvider>, context: EvaluationContext, resolutions: BaseProviderResolutionResult<T, TProviderStatus, TProvider>[]): BaseFinalResult<T, TProviderStatus, TProvider>;
}
 ...
 * @template TData - A record type that defines the shape of the stored data
 */
declare class MapHookData<TData = Record<string, unknown>> implements HookData<TData> {
 ...
 */
declare type AttributeValue = string | number | boolean | string[] | number[] | boolean[];
...
export { AggregateError, ClientProviderEvents as AllProviderEvents, ClientProviderStatus as AllProviderStatus, ComparisonStrategy as BaseComparisonStrategy, BaseEvaluationStrategy, FirstMatchStrategy as BaseFirstMatchStrategy, FirstSuccessfulStrategy as BaseFirstSuccessfulStrategy, ClientProviderEvents, ClientProviderStatus, DefaultLogger, ErrorCode, ErrorWithCode, FlagNotFoundError, GeneralError, GenericEventEmitter, InvalidContextError, LOG_LEVELS, MapHookData, OpenFeatureCommonAPI, OpenFeatureError, ParseError, ProviderFatalError, ProviderNotReadyError, ProviderWrapper, SafeLogger, ServerProviderEvents, ServerProviderStatus, StandardResolutionReasons, StatusTracker, TargetingKeyMissingError, TelemetryAttribute, TelemetryFlagMetadata, TypeMismatchError, constructAggregateError, createEvaluationEvent, instantiateErrorByErrorCode, isObject, isString, objectOrUndefined, statusMatchesEvent, stringOrUndefined, throwAggregateErrorFromPromiseResults };
export type { AnyProviderEvent, AttributeValue, BaseFinalResult, BaseHook, BaseProviderResolutionErrorResult, BaseProviderResolutionResult, BaseProviderResolutionSuccessResult, BaseStrategyPerProviderContext, BaseStrategyProviderContext, BeforeHookContext, BooleanFlagKey, ClientMetadata, CommonEventDetails, CommonProvider, ConfigChangeEvent, ErrorEvent, EvaluationContext, EvaluationContextValue, EvaluationDetails, EvaluationLifeCycle, EventContext, EventDetails, EventHandler, EventMetadata, EventOptions, Eventing, FlagMetadata, FlagValue, FlagValueType, HookContext, HookData, HookHints, JsonArray, JsonObject, JsonValue, Logger, ManageContext, ManageLogger, Metadata, NotChangeEvents, NumberFlagKey, ObjectFlagKey, Paradigm, PrimitiveValue, ProviderEntryInput, ProviderEventEmitter, ProviderMetadata, ReadyEvent, ReconcilingEvent, RegisteredProvider, ResolutionDetails, ResolutionReason, StaleEvent, StrategyEvaluationContext, StringFlagKey, TrackingEventDetails, TrackingEventValue };
